...
export const Brand: React.FC = () => (
    <div className="flex flex-row items-center">
        <span className="text-blue-950 text-4xl font-black mr-1">
            <svg className="h-10 w-10" viewBox="0 0 400 400" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <span className="text-blue-950 text-xl font-black uppercase leading-none">Netra<br />Construction</span>
    </div>
)

export const SectionHeading: React.FC<any> = ({ title, subtitle, action, ...rest }) => (
    <div {...rest} className="scroll-mt-20 border-t py-16 px-4 md:px-16 flex flex-row items-center justify-between">
        <div>
            {title && <Text as="h2" className="border-l-4 pl-4 text-2xl text-blue-950 font-bold">
                {title}
            </Text>}
            {subtitle && <Text as="p" className="border-l-4 pl-4 text-md text-neutral-500">
                {subtitle}
            </Text>}
        </div>
